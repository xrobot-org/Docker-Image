...
RUN apt install -y --no-install-recommends git curl sudo wget zip make && apt install -y gcc-arm-none-eabi net-tools usbutils nano gdb

RUN apt install -y --no-install-recommends cmake ninja-build clang clangd clang-tidy gcc g++ python3-tk python3-pip && apt clean
